...
/*******Laser_Model*******/
void 
AMCLLaser::SetModelLikelihoodFieldLLL(double z_hit,
                                   double z_rand,
                                   double sigma_hit,
                                   double max_occ_dist)
{
  this->model_type = LASER_MODEL_LIKELIHOOD_FIELD_LLL;
  this->z_hit = z_hit;
  this->z_rand = z_rand;
  this->sigma_hit = sigma_hit;
...
double AMCLLaser::LikelihoodFieldModel_lll(AMCLLaserData *data,AMCLLaserData *data_LLL, pf_sample_set_t* set)
{
  ...
  for (j = 0; j < set->sample_count; j++)
  {
    ...
    for (i = 0; i < data->range_count; i += step)
    {
      ...
      dist=data_LLL->range_max;
    for (int i_LLL = 0; i_LLL < data_LLL->range_count; i_LLL ++)
    {
    
    if(isfinite(data_LLL->ranges[i][0]))
    {
      ...
      if(dist>=sqrt(a_LLL*a_LLL+b_LLL*b_LLL))
      {
      dist=sqrt(a_LLL*a_LLL+b_LLL*b_LLL);
      }
    }
      ...
      if(z>=dist){
      pz += self->z_hit * exp(-(dist * dist) / z_hit_denom);
      }
      else
      {
      pz += self->z_hit * exp(-(z * z) / z_hit_denom);
